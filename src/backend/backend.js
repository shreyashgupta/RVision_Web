...
app.post('/faculty/courseQuestions',(req,res)=>
{
		...
		connection.query(`SELECT QType, Score, Category, count(*) as HowMany
						FROM question
						  ...
						  rows.map(async(x,i)=>
						  {
							  ...
							  if(subEval && ans.answerImages.length==0 && ques.modelAnswerAvailable)
							  {
								  var score1;
									console.log("here ")
								  //evaluation 
								  score1=await fetch(`https://api.dandelion.eu/datatxt/sim/v1?token=6d7c5593cc3f44ca97fb7b3e7f44cb8a&text1=${ques.modelAnswer}&text2=${ans.answerText}`)
