...
app.get("/", async function (req, res) {
  res.send({ message: "API landing Page" });
});
app.listen(PORT);
app.post("/studentSignUp", (req, res) => {
  ...
  connection.query(
    `INSERT INTO student VALUES('${USN}','${Fname}','${Lname}','${Email}',md5('${Password}'),'${Image}','${Department}','${Phone_no}','${Semester}','${DoB}')`,
  ...
  for (let i = 0; i < Courses.length; i++) {
    connection.query(
      `INSERT INTO enrollment VALUES('${USN}','${Courses[i]}')`,
      function (err, rows, fields) {
        if (err) throw err;
        console.log("The solution is: ", rows);
      }
...
app.post("/studentSignIn", (req, res) => {
  ...
  // console.log(typeof(Name))
  connection.query(
    `SELECT * FROM student WHERE Email='${Email}' AND Password=md5('${Password}')`,
    function (err, rows, fields) {
      if (err) throw err;
...
app.post("/facultySignUp", (req, res) => {
  ...
  // // console.log(typeof(Name))
  connection.query(
    `INSERT INTO faculty VALUES('${FID}','${Fname}','${Lname}','${Email}',md5('${Password}'),'${Image}','${Department}','${Phone_no}','${DoB}')`,
    function (err, rows, fields) {
      if (err) throw err;
  ...
  for (let i = 0; i < Courses.length; i++) {
    connection.query(
      `INSERT INTO handles VALUES('${FID}','${Courses[i]}')`,
      function (err, rows, fields) {
        if (err) throw err;
        console.log("The solution is: ", rows);
      }
...
app.post("/facultySignIn", (req, res) => {
  ...
  // console.log(typeof(Name))
  connection.query(
    `SELECT * FROM faculty WHERE Email='${Email}' AND Password=md5('${Password}')`,
    function (err, rows, fields) {
      if (err) throw err;
      // console.log(rows[0].Password)
      if (rows.length) {
...
app.post("/getCourses", (req, res) => {
  ...
  // console.log(typeof(Name))
  connection.query(
    `SELECT CourseId FROM course WHERE Department_name='${Department}'`,
    function (err, rows, fields) {
      if (err) throw err;
      console.log(rows);
      if (rows.length) {
...
app.post("/faculty/getCourses", (req, res) => {
  ...
  // console.log(typeof(Name))
  connection.query(
    `SELECT CourseId FROM handles WHERE FID='${FID}'`,
    function (err, rows, fields) {
      if (err) throw err;
      console.log(rows);
      if (rows.length) {
...
app.post("/getCoursesName", (req, res) => {
  ...
  for (let i = 0; i < Courses.length; i++) {
    connection.query(
      `SELECT Name FROM course WHERE CourseId='${Courses[i].CourseId}'`,
      function (err, rows, fields) {
        if (err) throw err;
        ...
        // else
        // 	res.json("error");
        if (i == Courses.length - 1) res.json(ret);
      }
    );
  }
});
...
app.post("/faculty/createQuestion", (req, res) => {
  ...
  // console.log(typeof(Name))
  connection.query(
    `INSERT INTO question VALUES('${QID}','${Qtype}','${Score}','${Category}','${FID}','${CourseId}')`,
    function (err, rows, fields) {
      if (err) {
        console.log(err);
        res.json("failed");
...
app.post("/faculty/getQuestionCount", (req, res) => {
  ...
  connection.query(
    `SELECT count(QID) n FROM question
...
app.post("/faculty/courseQuestions", (req, res) => {
  ...
  connection.query(
    `SELECT QType, Score, Category, count(*) as HowMany
						FROM question
						WHERE CourseId='${CourseId}'
						group by Qtype, Score, Category;
							`,
    function (err, rows, fields) {
      if (err) throw err;
      // console.log(rows[0].Password)
...
app.post("/faculty/createTest", (req, res) => {
  ...
  connection.query(
    `INSERT INTO test VALUES('${TestID}','${CourseId}','${Semester}','${FID}','${Date}')`,
    function (err, rows, fields) {
      if (err) throw err;
      res.json("success");
    }
...
app.post("/student/submission", (req, res) => {
  ...
  connection.query(
    `INSERT INTO submission VALUES('${SubmissionID}','${USN}','${TestID}','0')`,
    function (err, rows, fields) {
      if (err) throw err;
    }
  );
  for (let i = 0; i < QIDs.length; i++) {
    ...
    connection.query(
      `INSERT INTO answer VALUES('${AnsIds[i]}','${USN}','${Qtypes[i]}','0')`,
      function (err, rows, fields) {
        if (err) throw err;
      }
    );
    connection.query(
      `INSERT INTO qas VALUES('${SubmissionID}','${AnsIds[i]}','${QIDs[i]}')`,
      function (err, rows, fields) {
        if (err) throw err;
      }
    );
    ...
    async function (err, rows, fields) {
      ...
      if (rows.length) {
        rows.map(async (x, i) => {
          ...
          if (
            subEval &&
            ans.answerImages.length == 0 &&
            ques.modelAnswerAvailable
          ) {
            var score1;
            console.log("here ");
            //evaluation
            score1 = await fetch(
              `https://api.dandelion.eu/datatxt/sim/v1?token=6d7c5593cc3f44ca97fb7b3e7f44cb8a&text1=${ques.modelAnswer}&text2=${ans.answerText}`
            )
          ...
          if (subEval) {
            ...
            const res = await ansref
              .update({ studentScore: score })
              .catch(function (error) {
...
app.post("/student/report/getSummary", (req, res) => {
  let { TestID } = req.body;
  connection.query(
    `SELECT TestID, COUNT(distinct USN) AS attemptedBy, MAX(Score) AS maxScore, AVG(Score) AS avgScore, MIN(Score) AS minScore
		FROM submission
		WHERE TestID='${TestID}';
							`,
    function (err, rows, fields) {
...
app.post("/changePassword", (req, res) => {
  ...
  if (Student) {
    connection.query(
      ...
      function (err, rows, fields) {
        if (err) throw err;
        if (rows.affectedRows) res.json("success");
        else res.json("error");
      }
    );
  } else {
...
app.post("/faculty/fts/updateTest", (req, res) => {
  ...
  connection.query(
    `select TestID, count(distinct USN) 
